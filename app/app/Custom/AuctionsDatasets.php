...
class AuctionsDatasets
{
	public $dd = [
		'auctions' => [
			...
			'fapireq' => '/get/auctions',
			'hdrs' => ['Title', 'Time Left', 'Auction Ends', 'Current Price', '# of Bids', 'Source', 'Description'],
			'flds' => [
					'function (r) { return `<a href="${r["URL"]}" target="_blank">${r["Title"]}</a>` }',
					'"Time Left"', 
					'function (r) { return r["Auction Ends"].replace("T", " ").replace(".000Z", "") }',
					'"Current Price"', '"# of Bids"', 
					'function (r) { return r["URL"].replace("https://www.p", "P").replace(/\/.*/g, "") }',
