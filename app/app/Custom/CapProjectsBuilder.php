...
class CapProjectsBuilder
{
	static function build($prjs, $milestones)
	{
		...
		foreach ($dd as $i=>$d)
		{
			...
			$rr[$i] = [
				...
				'#CITY_PRIOR_ACTUAL' => ($d['CITY_PRIOR_ACTUAL'] ?? '') == '' ? '' : self::toFinShortK($d['CITY_PRIOR_ACTUAL'], 1000),
				'#NONCITY_PRIOR_ACTUAL' => ($d['NONCITY_PRIOR_ACTUAL'] ?? '') == '' ? '' : self::toFinShortK($d['NONCITY_PRIOR_ACTUAL'], 1000),
				...
				'#budget .current' => sprintf('<span data-content="%s">%s</span>', 
						...
						self::budgetRound($d['CITY_PLAN_TOTAL'] + $d['NONCITY_PLAN_TOTAL'], 1000), 
						self::toFinShortK($d['CITY_PLAN_TOTAL'] + $d['NONCITY_PLAN_TOTAL'], 1000)),
	...
	static function genBudgetLines($budgLines, $blRel)
	{
		$rr = [];
		$budgLines = explode('  ', $budgLines);
		$blRel = explode(' ', $blRel);
		foreach ($budgLines as $i=>$b)
		{
			$u = route('budgetLine', ['blcode' => $b]);
			$rr[] = $blRel[$i]
				? "<span class='badge badge-bl px-2 mr-2'><a href=\"{$u}\">{$b}</a></span>"
				: $b;
		}
		return implode(' ', $rr);
	}
